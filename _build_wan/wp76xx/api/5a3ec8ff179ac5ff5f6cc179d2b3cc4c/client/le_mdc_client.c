...
)
{
    ...
    if (!le_mdc_UnpackConState( &_msgBufPtr, &_msgBufSize,
                                               &ConnectionState ))
    {
        goto error_unpack;
    }
...
)
{
    ...
    if (connectionStatePtr &&
        (!le_mdc_UnpackConState( &_msgBufPtr, &_msgBufSize,
                                               connectionStatePtr )))
    {
        goto error_unpack;
    }
...
)
{
    ...
    if (downlinkDataBearerTechPtrPtr &&
        (!le_mdc_UnpackDataBearerTechnology( &_msgBufPtr, &_msgBufSize,
...
)
{
    ...
    LE_ASSERT(le_pack_PackReference( &_msgBufPtr, &_msgBufSize,
                                                  profileRef ));
    LE_ASSERT(le_mdc_PackPdp( &_msgBufPtr, &_msgBufSize,
                                                  pdp ));
...
)
{
    ...
    // Unpack the result first
    if (!le_mdc_UnpackPdp( &_msgBufPtr, &_msgBufSize, &_result ))
    {
        goto error_unpack;
    }
...
)
{
    ...
    LE_ASSERT(le_pack_PackReference( &_msgBufPtr, &_msgBufSize,
                                                  profileRef ));
    LE_ASSERT(le_mdc_PackAuth( &_msgBufPtr, &_msgBufSize,
                                                  type ));
    LE_ASSERT(le_pack_PackString( &_msgBufPtr, &_msgBufSize,
                                  userName, 64 ));
    LE_ASSERT(le_pack_PackString( &_msgBufPtr, &_msgBufSize,
...
)
{
    ...
    // Unpack any "out" parameters
    if (typePtr &&
        (!le_mdc_UnpackAuth( &_msgBufPtr, &_msgBufSize,
                                               typePtr )))
    {
        goto error_unpack;
    }
...
)
{
    ...
    // Unpack the result first
    if (!le_mdc_UnpackDisconnectionReason( &_msgBufPtr, &_msgBufSize, &_result ))
    {
        goto error_unpack;
    }
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_mdc_GetDisconnectionReasonExt;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_mdc_GetPlatformSpecificDisconnectionCodeExt;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_mdc_GetPlatformSpecificFailureConnectionReasonExt;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_mdc_GetProfileList;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;

    // Pack a list of outputs requested by the client.
    uint32_t _requiredOutputs = 0;
    _requiredOutputs |= ((!!(profileListPtr)) << 0);
    LE_ASSERT(le_pack_PackUint32(&_msgBufPtr, &_msgBufSize, _requiredOutputs));

    // Pack the input parameters
    if (profileListPtr)
    {
        LE_ASSERT(le_pack_PackSize( &_msgBufPtr, &_msgBufSize, (*profileListSizePtr) ));
    }
    ...
    if (profileListPtr)
    {
            LE_PACK_UNPACKSTRUCTARRAY( &_msgBufPtr, &_msgBufSize,
                             ...
                             255, le_mdc_UnpackProfileInfo,
                             &profileListResult );
        if (!profileListResult)
        {
        goto error_unpack;
        }
    }

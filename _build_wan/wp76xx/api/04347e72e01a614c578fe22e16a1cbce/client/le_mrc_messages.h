...
#define PROTOCOL_ID_STR "ca5b83e2431050ab61219b1665379807"
...
)
{
    ...
    LE_ASSERT(valuePtr);
    subResult = le_mrc_PackRat( bufferPtr, sizePtr,
                                                 valuePtr->rat );
    ...
    subResult = le_pack_PackString( bufferPtr, sizePtr,
                                    valuePtr->mnc, 4);
    result = result && subResult;
    subResult = le_mrc_PackNetRejCause( bufferPtr, sizePtr,
                                                 valuePtr->cause );
    result = result && subResult;
    subResult = le_mrc_PackNetRegDomain( bufferPtr, sizePtr,
                                                 valuePtr->rejDomain );
    result = result && subResult;
...
)
{
    ...
    if (result)
    {
        result = le_mrc_UnpackRat(bufferPtr, sizePtr,
                                                   &valuePtr->rat );
    }
    if (result)
    {
    ...
    if (result)
    {
        result = le_mrc_UnpackNetRejCause(bufferPtr, sizePtr,
                                                   &valuePtr->cause );
    }
    if (result)
    {
        result = le_mrc_UnpackNetRegDomain(bufferPtr, sizePtr,
                                                   &valuePtr->rejDomain );
    }
    return result;
}

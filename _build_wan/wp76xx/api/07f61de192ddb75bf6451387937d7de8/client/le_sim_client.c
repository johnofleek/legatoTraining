...
)
{
    ...
    le_sim_Id_t simId;
    if (!le_sim_UnpackId( &_msgBufPtr, &_msgBufSize,
                                               &simId ))
    {
        goto error_unpack;
    }
    le_sim_States_t simState;
    if (!le_sim_UnpackStates( &_msgBufPtr, &_msgBufSize,
                                               &simState ))
    {
        goto error_unpack;
    }
...
)
{
    ...
    if (!le_sim_UnpackStkEvent( &_msgBufPtr, &_msgBufSize,
                                               &stkEvent ))
    {
        goto error_unpack;
    }
...
)
{
    ...
    // Unpack the result first
    if (!le_sim_UnpackId( &_msgBufPtr, &_msgBufSize, &_result ))
    {
        goto error_unpack;
    }
...
)
{
    ...
    // Pack the input parameters
    LE_ASSERT(le_sim_PackId( &_msgBufPtr, &_msgBufSize,
                                                  simId ));
...
)
{
    ...
    // Unpack the result first
    if (!le_sim_UnpackStates( &_msgBufPtr, &_msgBufSize, &_result ))
    {
        goto error_unpack;
    }
...
)
{
    ...
    LE_ASSERT(le_sim_PackId( &_msgBufPtr, &_msgBufSize,
                                                  simId ));
    LE_ASSERT(le_sim_PackManufacturer( &_msgBufPtr, &_msgBufSize,
                                                  manufacturer ));
...
)
{
    ...
    // Unpack any "out" parameters
    if (refreshModePtr &&
        (!le_sim_UnpackStkRefreshMode( &_msgBufPtr, &_msgBufSize,
                                               refreshModePtr )))
    {
        goto error_unpack;
    }
...
)
{
    ...
    // Unpack any "out" parameters
    if (refreshStagePtr &&
        (!le_sim_UnpackStkRefreshStage( &_msgBufPtr, &_msgBufSize,
                                               refreshStagePtr )))
    {
        goto error_unpack;
    }
...
)
{
    ...
    LE_ASSERT(le_sim_PackId( &_msgBufPtr, &_msgBufSize,
                                                  simId ));
    LE_ASSERT(le_sim_PackCommand( &_msgBufPtr, &_msgBufSize,
                                                  command ));
    LE_ASSERT(le_pack_PackString( &_msgBufPtr, &_msgBufSize,
                                  fileIdentifier, 4 ));
